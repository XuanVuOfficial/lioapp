...
export const StaffList: React.FC<Props> = ({ users, departments, currentUser }) => {
    ...
    if (file) {
      try {
        ...
        if (result.success && result.data && result.data.avatar_100) {
          const avatarUrl = result.data.avatar_100;
          if (isEdit) {
  ...
  const handleCheckZalo = async () => {
    if (!newUser.email || !newUser.displayName || !newUser.password || !newUser.phone || !newUser.hireDate) {
      alert('Vui lòng điền đầy đủ Họ tên, Email, Mật khẩu, Số điện thoại và Ngày nhận việc');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Zalo User Preview & Confirmation Modal for Create */}
      {zaloPreviewData && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          ...
          >
            <div className="w-20 h-20 rounded-full mx-auto mb-4 overflow-hidden border-4 border-blue-100 shadow-md bg-slate-100 flex items-center justify-center">
              {zaloPreviewData.avatar ? (
                <img src={zaloPreviewData.avatar} alt="Zalo Avatar" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              ) : (
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-1">{zaloPreviewData.zalo_name}</h3>
            <p className="text-xs text-slate-500 font-mono mb-4">ID Zalo: {zaloPreviewData.useridzalo}</p>

            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-left space-y-2 mb-6">
              <p className="text-xs text-slate-500"><strong className="text-slate-700">SĐT Zalo:</strong> {newUser.phone}</p>
              <p className="text-xs text-slate-500"><strong className="text-slate-700">Họ và tên:</strong> {newUser.displayName}</p>
              <p className="text-xs text-slate-500"><strong className="text-slate-700">Email:</strong> {newUser.email}</p>
              <p className="text-xs text-slate-500"><strong className="text-slate-700">Vai trò:</strong> {getSystemRole(newUser.role)}</p>
              <p className="text-xs text-slate-500"><strong className="text-slate-700">Phòng ban:</strong> {departments.find(d => d.id === newUser.departmentId)?.name || 'Chưa chọn'}</p>
              <p className="text-xs text-slate-500"><strong className="text-slate-700">Ngày nhận việc:</strong> {new Date(newUser.hireDate).toLocaleDateString('vi-VN')}</p>
            </div>

            <div className="flex gap-3">
              <button
                onClick={() => setZaloPreviewData(null)}
                className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50 transition-all cursor-pointer"
              >
                Hủy
              </button>
              <button
                onClick={handleConfirmCreateWithZalo}
                disabled={isLoading}
                className="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 cursor-pointer"
              >
